...
Notice that `StartCoroutine` was called before `IsolatedCallTest_` was executed even though it looks like `IsolatedCallTest_` was called first. That is because `IsolatedCallTest_` is not actually a method - it is an instantiation of a compiler generated class that implements IEnumerator.
